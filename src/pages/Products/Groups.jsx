...
import SubGroupsSlider from '../../components/Slider/SubGroupsSlider.jsx';
...
const [categories , setCategories] = useState([]);
  ...
  async function GetProductsByGroup() {
    try {
      ...
      const response = await ProductsService.GetProductsWithGroup(auth_key , user_id , id , start , limit);
      if(response.status){
      ...
      console.log("GetProductsByGroup", response.data);
    } catch (error) {
  ...
  return (
    ...
    <section className='AllProductsSection'>
      ...
      { SubCategories.length>0&&
        <SubGroupsSlider categories={SubCategories||[]}/>        
      }
